...
from hashs import quebrar_chave, converter_wif, aguarda_quebra, quebrar_pollard
...
def main(address, destino, kangaroo:bool, gpu:bool):
...
if __name__ == '__main__':
    limpeza()
    print('---\nIniciando Script, Atenção, esse script monitora as transações da carteira "alvo" e usa GPU pra quebrar a chave pública na privada, converte em WIF e tenta realizar uma transferencia para sua conta, o código ainda está em desenvolvimento, esteja pronto para transferir manualmente caso falhe.\n---\n')
    sim = ['s', 'sim', 'y', 'yes']
    ...
    if input('Deseja usar o Kangaroo, (s/n): ').lower() in sim:
        usar_kangaroo = True # Se True usa o Kangaroo, se False usa o Collider
    else:
        usar_kangaroo = False
    usar_gpu = False
    if usar_kangaroo:
    ...
    main(address, destino, usar_kangaroo, usar_gpu)
